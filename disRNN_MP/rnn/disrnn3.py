...
class hkDisRNN(hk.RNNCore):
    ...
    def __call__(
        ...
        latent_sigmas_param = hk.get_parameter(
            'latent_sigmas_param',
            (self._latent_size,),
            init=hk.initializers.Constant(constant=0),
        )
        _latent_sigmas = (
            (self._epsilon + jnp.abs(latent_sigmas_param)) * (1 - self._eval_mode)
        )
        
        # choiceMLP input bottleneck
        choice_mlp_sigmas_param = hk.get_parameter(
            'choice_mlp_sigmas_param',
            (self._latent_size,),
            init=hk.initializers.Constant(constant=0),
        )
        _choice_mlp_sigmas = (
            (self._epsilon + jnp.abs(choice_mlp_sigmas_param)) * (1 - self._eval_mode)
        )
