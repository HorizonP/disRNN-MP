from typing import Iterable, Callable, Any, List, Literal, Optional, Tuple, Sequence, get_args
...
from .kl import kl_gaussian_mu_sigma
from disRNN_MP.metrics import BerLL_logit
from disRNN_MP.rnn.utils import RNNtransformed, transform_hkRNN, make_train_step_with_loss_fun

_TYPE_sigma_parameterization = Literal['abs', 'sigmoid']

# TODO add an `asdict` method that provide dictionary of all parameters required for reproducibility
class hkDisRNN(patchable_hkRNNCore):
    ...
    - KL-divergence of the information bottleneck is now calculated in the correct way
    ...
    def _configure_latent_bn(self):
        ...
        if self._latent_bn_gate:
            def latent_bn(latent):
                self._latent_sigmas = self.get_latent_sigma() # type: ignore

                self._latent_multipliers = hk.get_parameter( # type: ignore
                    'latent_gates',
                    (self._latent_size,),
                ...
                noisy_latent = latent * self._latent_multipliers + self._latent_sigmas * jax.random.normal( # type: ignore
                    hk.next_rng_key(), latent.shape
                )

                penalty = kl_gaussian_mu_sigma(latent * self._latent_multipliers, self._latent_sigmas) # type: ignore
            ...
            def latent_bn(latent):
                self._latent_sigmas = self.get_latent_sigma() # type: ignore
                self._latent_multipliers = jnp.ones((self._latent_size,)) # type: ignore

                # noised_up_latents: (batch_size, latent_size)
                noisy_latent = latent + self._latent_sigmas * jax.random.normal( # type: ignore
                    hk.next_rng_key(), latent.shape
                )

                penalty = kl_gaussian_mu_sigma(latent, self._latent_sigmas) # type: ignore
    ...
    def choice_selection(self, noised_up_latents: Array) -> Tuple[jax.Array, jax.Array]:
        ...
        penalty = kl_gaussian_mu_sigma(noised_up_latents * _choice_mlp_multipliers, _choice_mlp_sigmas)
...
def make_transformed_disrnn(
        target_size: int = 2,
        latent_size: int = 10,
        update_mlp_shape: Sequence[int] = (10, 10, 10),
        choice_mlp_shape: Sequence[int] = (10, 10, 10),
        eval_mode: bool = False,
        sigma_parameterization: Optional[_TYPE_sigma_parameterization] = "abs",
        epsilon: float = 1e-4,
        latent_bn_gate: Optional[bool] = False,
    ) -> RNNtransformed:
    """Create a transformed disRNN model ready for training or evaluation.

    Args:
        target_size: network output size. Defaults to 2.
        latent_size: latent size. Defaults to 10.
        update_mlp_shape: shape of update MLP. Defaults to (10, 10, 10).
        choice_mlp_shape: shape of choice MLP. Defaults to (10, 10, 10).
        eval_mode: eval_mode will turn off randomness. Defaults to False.
        sigma_parameterization: how to parameterize sigma ('abs' or 'sigmoid').
        epsilon: minimum sigma for bottleneck.
        latent_bn_gate: whether to use a mean multiplier gate for latent bottleneck.

    Returns:
        RNNtransformed: transformed model with init, apply, get_initial_state, model_haiku.
    """
    def disrnn_module():
        model = hkDisRNN(
...
# ============= Loss and training convenience functions =============
...
def make_loss_fun(
    ...
    """Create a penalized categorical loss function for disRNN training.

    Args:
        model_apply (Callable): the model apply function
        penalty_scale: weight for the information bottleneck penalty
        beta_scale: scale for update rule bottleneck penalties
        theta_scale: scale for choice MLP input bottleneck penalties
...
def make_param_metric_expLL(model: RNNtransformed, ys_feat_ind: int = 0, prediction_slice: slice = slice(None)):
    """Create a parameter metric function that computes exponentiated normalized log-likelihood."""
...
def make_train_step(model_apply, optimizer, penalty_scale=0, beta_scale=1, theta_scale=1):
    """Create a train step function using the disRNN loss function.

    Convenience wrapper around `make_train_step_with_loss_fun` with `make_loss_fun`.

    Args:
        model_apply: the model apply function
    ...
    Returns:
        train_step function
    """
    return make_train_step_with_loss_fun(
        model_apply, optimizer, make_loss_fun,
        penalty_scale=penalty_scale, beta_scale=beta_scale, theta_scale=theta_scale)
